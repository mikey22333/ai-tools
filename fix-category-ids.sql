-- SAFE FIX FOR CATEGORY ID MISMATCHES
-- This script will identify and fix only the problematic category IDs

-- Step 1: Check current state
SELECT 'CURRENT CATEGORIES TABLE' as info, id, name FROM categories ORDER BY id;

-- Step 2: Find categories that don't have matching tools
SELECT 
  'CATEGORIES WITH NO MATCHING TOOLS' as issue,
  c.id as category_id,
  c.name,
...
-- Step 3: Check if tools categories already exist in categories table
SELECT 
  'TOOLS CATEGORY IDS' as info,
  DISTINCT category_id,
  CASE 
    WHEN EXISTS (SELECT 1 FROM categories WHERE id = tools.category_id) 
...
-- Step 4: Insert missing categories instead of updating
...
-- Step 5: Update tool counts for all categories
UPDATE categories 
SET tool_count = (
  SELECT COUNT(*) 
  FROM tools 
...
-- Step 6: Remove old categories that have no tools
DELETE FROM categories 
WHERE tool_count = 0 AND id NOT IN (SELECT DISTINCT category_id FROM tools);
